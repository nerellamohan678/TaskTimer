...
class TaskTimerViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private val settingListener =
        SharedPreferences.OnSharedPreferenceChangeListener { sharedPreferences, s ->
            when (s) {
                SETTINGS_IGNORE_LESS_THAN -> {
                    ignoreLessThan = sharedPreferences.getInt(s, SETTINGS_DEFAULT_IGNORE_LESS_THAN)
                    Log.d(
                        TAG,
                        "settingListener: now ignoring timings less than $ignoreLessThan seconds"
                    )
    ...
    private fun loadTasks() {
        //below code is for fetching data from content provider
    ...
    fun startEditing(taskId: Long) {
        if (BuildConfig.DEBUG && editedTaskId != 0L) throw IllegalStateException(
            "startEditing called without stopping edit. editedTaskId: $editedTaskId, taskId: $taskId"
        )
        editedTaskId = taskId
    }
    ...
    fun timeTask(task: Task) {
        ...
        // Update the LiveData
        taskTiming.value = if (currentTiming != null) task.name else "zero"
    }
